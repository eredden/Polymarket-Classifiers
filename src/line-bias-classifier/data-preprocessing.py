...
# Determine the sport category for a given row.
def categorize_sport_refined(row):
    text = (str(row["question"]) + " " + str(row["description"])).lower()
    
    # Esports check first (generic terms might overlap).
    if "games total" in text or "maps" in text or "best of 3" in text:
        return "esports"
    
    if "nba" in text or "basketball" in text or "cbb" in text or "ncaa men" in text:
        if "1h" in text or "first half" in text:
            return "basketball_first_half"
        
        return "basketball"
    elif "nfl" in text or "football" in text or "cfb" in text:
        # Distinguish player props vs game totals.
        if "passing yards" in text or "rushing yards" in text or "receiving yards" in text:
             return "football_player_prop"
        
        return "football"
    elif "soccer" in text or "epl" in text or "premier league" in text or "champions league" in text or "la liga" in text or "serie a" in text or "bundesliga" in text or "ligue 1" in text or "mls" in text or "brasileirão" in text:
        return "soccer"
    elif "nhl" in text or "hockey" in text:
        return "hockey"
...
def preprocess_data(data: pandas.DataFrame) -> pandas.DataFrame:
    ...
    # We convert the sports categories into one-hot encoding columns to
    # prevent XGBoost from mistakenly believing that one sport is greater than
    # another -- they are nominal values, not ordinal.
    data = pandas.get_dummies(data, columns=['sport'], prefix='sport')
